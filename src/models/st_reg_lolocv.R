...
# Train the model
lm <- lm(pm2_5_mean ~ address_density + pop_density + log_address_density + log_pop_density + road + rail + easting + northing + DD + HH + FH + `T` + P + U + traffic_signals, data=d, na.action="na.exclude")
summary(lm)
...
crossStat <- function(pred=pred_matrix[!is.na(res_matrix)], res=res_matrix[!is.na(res_matrix)], digits=NA) {
...
filename = "C:/Users/Klant/Documents/GitHub/ADS-Snuffelfiets-Thesis/output/lolocv/lolocv_1000.csv"
